...
module.exports = async (ctx) => {
  try {
    ...
    if (!input && !replyMsg) {
      return ctx.reply(
        isIndo
          ? "💬 Gunakan format: /farelgpt <pesan> atau balas pesan dengan /farelgpt <teks>"
          : "💬 Use format: /farelgpt <message> or reply to a message with /farelgpt <text>",
        { reply_to_message_id: ctx.message?.message_id }
      );
    }
    ...
    // Call endpoint Tifa
    const url = `https://endpoint-hub.up.railway.app/api/chatai/farel-gpt?text=${encodeURIComponent(promptText)}`;
    ...
    console.error("❌ Error di /farelgpt:", err);
    ctx.reply(
      (ctx.from?.language_code || "").startsWith("id")
        ? "⚠️ Terjadi kesalahan saat memproses /farelgpt 😥"
        : "⚠️ An error occurred while processing /farelgpt 😥",
      { reply_to_message_id: ctx.message?.message_id }
    );
  }
};
