...
module.exports = async (ctx) => {
  try {
    ...
    if (!input && !replyMsg) {
      return ctx.reply(
        isIndo
          ? "💬 Gunakan format: /navix <pesan> atau balas pesan dengan /navix <teks>"
          : "💬 Use format: /navix <message> or reply to a message with /navix <text>",
        { reply_to_message_id: ctx.message?.message_id }
      );
    }
    ...
    // Call endpoint navix
    const url = `https://endpoint-hub.up.railway.app/api/chatai/navix?text=${encodeURIComponent(promptText)}`;
    ...
    console.error("❌ Error di /navix:", err);
    ctx.reply(
      (ctx.from?.language_code || "").startsWith("id")
        ? "⚠️ Terjadi kesalahan saat memproses /navix 😥"
        : "⚠️ An error occurred while processing /navix 😥",
      { reply_to_message_id: ctx.message?.message_id }
    );
  }
};
