...
module.exports = async (ctx) => {
  try {
    // ambil soal dari API
    const res = await fetch("https://api.deline.web.id/game/tebakgame");
    ...
    global.gameSession.set(sent.message_id, {
      type: "tebakgame",      // penting untuk handler
      userId: ctx.from.id,
    ...
    console.error("❌ TebakGame Error:", err);
    ctx.reply("❌ Gagal mengambil soal Tebak Game.");
  }
};
