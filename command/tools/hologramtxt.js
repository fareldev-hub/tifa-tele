...
module.exports = async (ctx) => {
  try {
    ...
    if (!input) {
      return ctx.reply(
        isIndo
          ? "💡 Gunakan: /hologramtxt <teks>\nContoh: /hologramtxt Halo Dunia"
          : "💡 Usage: /hologramtxt <text>\nExample: /hologramtxt Hello World",
    ...
    const apiUrl = `https://api.nekolabs.web.id/ephoto/3d-hologram-text?text=${encodeURIComponent(
      input
    )}`;
    ...
    console.error("Error /hologramtxt:", err);
    await ctx.reply(
