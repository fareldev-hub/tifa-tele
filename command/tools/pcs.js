...
module.exports = async (ctx) => {
  try {
    ...
    if (!input) {
      return ctx.reply(
        isIndo
          ? "💡 Gunakan: /pcs <teks>\nContoh: /pcs Halo Dunia"
          : "💡 Usage: /pcs <text>\nExample: /pcs Hello World",
    ...
    const apiUrl = `https://api.nekolabs.web.id/ephoto/3d-paper-cut-style?text=${encodeURIComponent(
      input
    )}`;
