...
module.exports = async (ctx) => {
  try {
    ...
    if (!ctx.message.reply_to_message || !ctx.message.reply_to_message.photo) {
      return ctx.reply(
        isIndo
          ? "📸 Balas gambar dengan /toghibli untuk mengubahnya ke gaya ghibli."
          : "📸 Reply to an image with /toghibli to convert it to ghibli style.",
        { reply_to_message_id: ctx.message?.message_id }
      );
    }
    ...
    const apiUrl = `https://api.nekolabs.web.id/tools/convert/toghibli/v1?imageUrl=${encodeURIComponent(publicUrl)}`;
    const res = await fetch(apiUrl);
    const data = await res.json();

    if (!data?.success || !data?.result) {
      throw new Error("❌ Gagal mengubah gambar ke ghibli.");
    }

    // Unduh hasil dari "result"
    const ghibliRes = await fetch(data.result);
    const ghibliBuffer = Buffer.from(await ghibliRes.arrayBuffer());
    ...
    await ctx.replyWithPhoto(
      ...
      {
        caption: isIndo
          ? "✅ *Berhasil diubah ke versi ghibli!*"
          : "✅ *Successfully converted to ghibli style!*",
        parse_mode: "Markdown",
        reply_to_message_id: ctx.message?.message_id,
      }
    );
    ...
    console.error("❌ Error di /toghibli:", err);
    await ctx.reply(
